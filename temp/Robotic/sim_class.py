...
class Simulation:
    def __init__(self, num_agents, render=True, rgb_array=False):
        ...
        test = observation["robotId_1"]["pipette_position"]

        #self.update_camera(test)
    ...
    def apply_actions(self, actions): # actions [[x,y,z,drop], [x,y,z,drop], ...
        for i in range(len(self.robotIds)):
            ...
            observation = self.get_states()
            test = observation["robotId_2"]["pipette_position"]
    ...
    def drop(self, robotId):
        ...
        sphereColor = [0, 0, 1, 0.5]  # RGBA (Red in this case)
