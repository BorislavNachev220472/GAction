...
# Predict root, shoot, and occlusion mask
def predict_image(image, patch_size, segmentation_model, occlusion_inpainter, shoot_model, refinement_steps=10):
...
def save_model_predictions(input_folder, root_segmentation_model, occlusion_inpainter, shoot_segmentation_model, padder, output_folder, refinement_steps=5):
    """
    Saves predictions from root, shoot, and occlusion segmentation models for each image in the specified input folder.

    Parameters:
    - input_folder: Path to the folder containing input images.
    - root_segmentation_model: Pre-loaded root segmentation model.
    - occlusion_inpainter: Model or function for handling occlusions (inpaint them).
    - shoot_segmentation_model: Pre-loaded shoot segmentation model.
    - padder: Function to pad images to the required size.
    - refinement_steps: Number of times to apply the occlusion inpainting.
    """
    ...
    for filename in os.listdir(input_folder):
        if filename.endswith(".png") or filename.endswith(".tif") or filename.endswith(".jpg"):
            ...
            root_mask, shoot_mask, occlusion_mask = predict_image(
                image, 256, root_segmentation_model, occlusion_inpainter, shoot_segmentation_model, refinement_steps=refinement_steps
            )

            # TODO 
            # Create a new mask which is the combination of root and occlusion masks
            # Save it with a specific name ending sich as full_root.png
            # Change process_image_rois to use this mask instead of the root mask
